...
import { 
  ...
  FaMapMarkerAlt, FaRobot, FaVideo, FaChartBar
} from "react-icons/fa";
import { 
...
function DonezoPatientDashboard() {
  ...
  const [activeMedicationTab, setActiveMedicationTab] = useState('today'); // 'today' ou 'history'
  const [medicationHistory, setMedicationHistory] = useState([]); // Historique
...
const chargerStats = async () => {
  try {
    ...
    const aVenir = tousRdv.filter(r => 
      (r.statut === "CONFIRMED" || r.statut === "PENDING") &&
      new Date(r.date) >= maintenant
    );

    const passes = tousRdv.filter(r => new Date(r.date) < maintenant);

    const consultations = await consultationAPI.getConsultations();
    const onlineConsultations = Array.isArray(consultations?.data)
      ? consultations.data.filter(c => c.type === "teleconsultation").length
      : 0;

    setStats({
      upcomingAppointments: aVenir.length,
      pastAppointments: passes.length,
      doctors: 5, // à remplacer par un vrai count plus tard
      messages: onlineConsultations
    });
  } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const medsToday = await medicationService.getToday();
          setMedications(medsToday);
        } catch (err) {
          console.error("Erreur médicaments du jour:", err);
          setMedications([]);
        }
  ...
  const fetchMeds = async () => {
    try {
      const res = await medicationService.getToday();
      setMedications(res);
    } catch (e) {
      console.log("Erreur médicaments du jour", e);
    }
  };
  fetchMeds();
}, []);
...
useEffect(() => {
  const fetchMedicationHistory = async () => {
    try {
      const history = await medicationService.getHistory(); // Assure-toi que tu as une méthode pour ça
      setMedicationHistory(history);
    } catch (err) {
      console.error("Erreur récupération historique médicaments :", err);
  ...
  const quickAccessButtons = [
    ...
    { title: "Prescription médecin", icon: <FaPrescriptionBottleAlt />, color: "bg-purple-100 text-purple-600", onClick: () => navigate("/patient/consultations") },
  ];
  ...
  try {
    await medicationService.markTakenToday(id);
    const updated = await medicationService.getToday();
    setMedications(updated);
  } catch (e) {
    console.error("Erreur prise médicament :", e);
  }
};
          ...
          <div className="space-y-6">
            ...
            <div className="dashboard-card p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-2">
                  <button
                    className={`btn btn-sm ${activeMedicationTab === 'history' ? 'btn-primary' : 'btn-outline-primary'}`}
                    onClick={() => setActiveMedicationTab('history')}
                  >
                    Aujourdh'hui
                  </button>
                </div>
              ...
              {/* Contenu onglets */}
  {activeMedicationTab === 'today' && (
    <div className="space-y-3">
      {medications.length === 0 ? (
        <p className="text-gray-500 italic">Aucun médicament pour l’instant</p>
      ) : (
        medications.map(m => (
          <div key={m.history_id} className="flex justify-between p-3 border rounded-lg">
            <div>
              <p className="font-semibold">{m.medicament}</p>
              <p className="text-sm text-gray-600">{m.dosage} — {m.heure}</p>
            </div>
            <button
              className={`px-3 py-1 rounded text-white ${m.taken ? "bg-gray-400" : "bg-green-600"}`}
              disabled={m.taken}
              onClick={() => handleMark(m.history_id)}
            >
              {m.taken ? "Pris ✅" : "Prendre"}
            </button>
          </div>
        ))
      )}
            ...
            <table className="table table-striped">
              ...
              <tbody>
                {medicationHistory.map(item => {
                  const priseEffectuee = !!item.prise_effectuee;
                  return (
                    <tr key={item.id}>
                      <td>{item.rappel_medicament || item.medicament || "N/A"}</td>
                      <td>{item.date_prise ? new Date(item.date_prise).toLocaleString('fr-FR') : "Date non définie"}</td>
                      <td>
                        {priseEffectuee ? (
                    ...
                    <div key={m.history_id} className="flex justify-between p-3 border rounded-lg">
                      ...
                      <button
                        className={`px-3 py-1 rounded text-white ${
                          m.taken ? "bg-gray-400" : "bg-green-600"
                        }`}
                        disabled={m.taken}
                        onClick={() => handleMark(m.history_id)}
                      >
